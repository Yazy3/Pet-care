<?php $pageTitle = 'New Vaccination Record'; require __DIR__ . '/../../layouts/layout.php'; ?>

<div class="row justify-content-center">
  <div class="col-12 col-lg-8">
    <div class="form-card">
      <div class="fc-title"><i class="bi bi-clipboard2-pulse-fill me-2 text-success"></i>Log Vaccination Record</div>
      <form method="POST" action="?controller=record&action=store">
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <select name="pet_id" class="form-select" required>
              ...
              <?php foreach($pets as $p): ?>
                <option value="<?= $p['pet_id'] ?>"><?= htmlspecialchars($p['label']) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-6">
            ...
            <select name="vaccine_id" class="form-select" required>
              ...
              <?php foreach($vaccines as $v): ?>
                <option value="<?= $v['vaccine_id'] ?>"><?= htmlspecialchars($v['label']) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-6">
            <label class="form-label fw-semibold">Date Administered <span class="text-danger">*</span></label>
            <input type="date" name="date_administer" class="form-control" value="<?= date('Y-m-d') ?>" required>
          </div>
          <div class="col-md-6">
          ...
          <div class="col-md-6">
            <label class="form-label fw-semibold">Administered By (Staff) <span class="text-danger">*</span></label>
        ...
        <div class="d-flex gap-2 mt-4">
          ...
          <a href="?controller=record&action=index" class="btn btn-outline-secondary">Cancel</a>
        </div>
      </form>
    </div>
  </div>
